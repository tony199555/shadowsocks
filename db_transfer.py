...
class TransferBase(object):
    ...
    def del_server_out_of_bound_safe(self, last_rows, rows):
        ...
        for row in rows:
            try:
                allow = switchrule.isTurnOn(row) and row['enable'] == 1 and row['flow_up'] + row['flow_down'] < row[
                    'transfer']
            except Exception as e:
                allow = False
...
class DbTransfer(TransferBase):
    ...
    def update_all_user(self, dt_transfer):
        ...
        for id in dt_transfer.keys():
            ...
            query_sub_when += ' WHEN %s THEN flow_up+%s' % (id, int(transfer[0] * self.cfg["transfer_mul"]))
            query_sub_when2 += ' WHEN %s THEN flow_down+%s' % (id, int(transfer[1] * self.cfg["transfer_mul"]))
...
class Dbv3Transfer(DbTransfer):
    ...
    def update_all_user(self, dt_transfer):
        ...
        for id in dt_transfer.keys():
            ...
            try:
                if id in self.port_uid_table:
                    cur.execute(
                        "INSERT INTO `user_traffic_log` (`id`, `user_id`, `flow_up`, `flow_down`, `node_id`, `rate`, `traffic`, `log_time`) VALUES (NULL, '" + \
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO `ss_node_info_log` (`id`, `node_id`, `uptime`, `load`, `log_time`) VALUES (NULL, '" + \
    ...
    def pull_db_users(self, conn):
        ...
        cur.execute(
            "SELECT " + ','.join(node_info_keys) + " FROM ss_node where `id`='" + str(self.cfg["node_id"]) + "'")
            ...
            for row in rows:
                if "port" in row:
                    ...
                    if port in dt_transfer:
                        row["flow_up"] += dt_transfer[port][0]
                        row["flow_down"] += dt_transfer[port][1]
